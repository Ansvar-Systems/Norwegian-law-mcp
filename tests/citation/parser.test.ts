...
describe('parseCitation', () => {
  ...
  describe('Norwegian LOV statutes', () => {
    it('should parse LOV citation with statute id only', () => {
    ...
    it('should parse LOV citation with section', () => {
      const result = parseCitation('LOV-2018-06-15-38 § 2');
      expect(result.valid).toBe(true);
      expect(result.document_id).toBe('LOV-2018-06-15-38');
      expect(result.section).toBe('2');
    });

    it('should parse LOV citation without trailing sequence', () => {
      const result = parseCitation('LOV-1967-02-10 § 5');
      expect(result.valid).toBe(true);
      expect(result.document_id).toBe('LOV-1967-02-10');
      expect(result.section).toBe('5');
    });

    it('should parse LOV citation with alphabetic suffix', () => {
      const result = parseCitation('LOV-1814-05-17-nn § 1');
      expect(result.valid).toBe(true);
      expect(result.document_id).toBe('LOV-1814-05-17-NN');
...
describe('detectDocumentType', () => {
  it('should detect statute', () => {
    ...
    expect(detectDocumentType('LOV-2018-06-15-38 § 1')).toBe('statute');
    expect(detectDocumentType('LOV-1967-02-10 § 5')).toBe('statute');
    expect(detectDocumentType('LOV-1814-05-17-nn § 1')).toBe('statute');
  });
