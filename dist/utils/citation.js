...
export function buildProvisionCitation(documentId, documentTitle, provisionRef, inputDocId, inputSection, sourceUrl, shortName) {
    ...
    if (documentId !== canonicalRef)
        aliases.push(documentId);
    if (documentTitle && documentTitle !== canonicalRef)
        aliases.push(documentTitle);
    return {
        canonical_ref: canonicalRef,
        display_text: displayText,
 ...
 */
export function buildRegulationCitation(reference, title, toolName, toolArgs, authority, sourceUrl) {
    const canonicalRef = reference;
    const displayText = title || reference;
    const aliases = [];
    if (authority)
        aliases.push(`${authority}: ${reference}`);
    return {
        canonical_ref: canonicalRef,
        display_text: displayText,
        ...(aliases.length > 0 && { aliases }),
