...
const SERVER_NAME = pkg.name.replace(/^@ansvar\//, '');
...
function resolveDbPath() {
    ...
    for (const [key, value] of Object.entries(process.env)) {
        if (key.endsWith('_LAW_DB_PATH') && value)
            return value;
    }
    // 2. Fall back to any *_DB_PATH env var
    for (const [key, value] of Object.entries(process.env)) {
        if (key.endsWith('_DB_PATH') && value)
            return value;
    }
    ...
    throw new Error(`Database not found. Set a *_DB_PATH env var or place database.db in data/`);
}
// ---------------------------------------------------------------------------
// Session management
// ---------------------------------------------------------------------------
...
async function main() {
    ...
    const aboutContext = { version: SERVER_VERSION, fingerprint, dbBuilt };
    ...
    const httpServer = createHttpServer(async (req, res) => {
        ...
        try {
            ...
            if (url.pathname === '/mcp') {
                const sessionId = validSessionId(req.headers['mcp-session-id']);
