 ...
 */
export function validateParsedCitation(db, parsed) {
    const warnings = [];
    // Check document existence
    const doc = db.prepare('SELECT id, title, status FROM legal_documents WHERE id = ?').get(parsed.document_id);
    if (!doc) {
        return {
            citation: parsed,
            document_exists: false,
    ...
    if ((parsed.type === 'statute' || parsed.type === 'regulation') && (parsed.chapter || parsed.section)) {
        const provisionRef = parsed.chapter && parsed.section
            ...
            : parsed.section || '';
        if (provisionRef) {
            const prov = db.prepare('SELECT id FROM legal_provisions WHERE document_id = ? AND provision_ref = ?').get(parsed.document_id, provisionRef);
