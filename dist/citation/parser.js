 ...
 */
export function detectDocumentType(citation) {
    const trimmed = citation.trim();
    // Norwegian LOV
    ...
    if (/^(?:hr|la|lb|le|ting)-\d{4}-/i.test(trimmed))
        return 'case_law';
    if (/^rt\.\s*\d{4}/i.test(trimmed))
        return 'case_law';
    // Proposition (Norwegian and Swedish)
    if (/^(?:prop\.?|ot\.prp\.?)/i.test(trimmed))
        return 'bill';
    // NOU
    if (/^nou\s/i.test(trimmed))
        return 'sou';
    // Legacy Swedish
    if (/^sou\s/i.test(trimmed))
        return 'sou';
    if (/^ds\s/i.test(trimmed))
        return 'ds';
    if (/^(nja|hfd|ad|md|mig)\s/i.test(trimmed))
        return 'case_law';
    if (/^(?:sfs\s+)?\d{4}:\d+/i.test(trimmed))
        return 'statute';
    return null;
}
