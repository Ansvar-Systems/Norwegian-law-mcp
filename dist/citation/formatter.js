 ...
 *   - pinpoint: "3 kap. 5 §"
 */
export function formatCitation(citation, format = 'full') {
    if (!citation.valid) {
        return citation.raw;
    }
...
function formatStatute(citation, format) {
    ...
    if (format === 'short') {
        if (chapter && section)
            return `${document_id} ${chapter}:${section}`;
        if (section)
            return isLov ? `${document_id} § ${section}` : `${document_id} ${section} §`;
        return document_id;
    }
    // full format
    let result = isLov ? `LOV ${document_id}` : `SFS ${document_id}`;
    if (chapter)
        result += isLov ? ` ${chapterLabel} ${chapter}` : ` ${chapter} ${chapterLabel}`;
    if (section)
        result += ` ${section} §`;
    return result;
}
function formatBill(citation) {
...
function formatCaseLaw(citation) {
    ...
    if (citation.document_id.startsWith('NJA')) {
        ...
        if (citation.page)
            parts.push(`ref. ${citation.page}`);
    }
    else {
        // Norwegian modern: HR-2020-1234-A avsnitt X
        if (citation.page)
            parts.push(`avsnitt ${citation.page}`);
    }
    return parts.join(' ');
}
/**
 ...
 */
export function formatProvisionRef(chapter, section) {
    if (chapter) {
