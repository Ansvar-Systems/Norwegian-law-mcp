...
function normalizeSectionRef(section) {
...
export function parseRiksdagenProvisions(text) {
    return parseLovdataProvisions(text);
}
export function parseLovdataProvisions(text) {
    const lines = text.split(/\r?\n/);
    const provisions = [];
    const seenProvisionRefs = new Set();
    const lastOrdinalByChapter = new Map();
    const diagnostics = {
    ...
    for (const rawLine of lines) {
        ...
        if (sectionMatch) {
            ...
            const isOutOfOrderFromCurrent = (currentOrdinal !== undefined &&
                candidateOrdinal !== undefined &&
                candidateOrdinal <= currentOrdinal);
            const isOutOfOrderFromHistory = (!chapterActivated &&
                lastOrdinal !== undefined &&
                candidateOrdinal !== undefined &&
                candidateOrdinal <= lastOrdinal);
            const isLikelyInlineReference = (!chapterActivated &&
                currentSection !== undefined &&
                currentContent.length > 0 &&
                startsWithLowercase(remainder));
            const isSuspiciousFlatJump = (!chapterActivated &&
                !chapterForSection &&
...
//# sourceMappingURL=lovdata-provision-parser.js.map