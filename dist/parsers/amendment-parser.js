 ...
 */
function parseLovReference(ref) {
    // Already a LOV id
 ...
 */
export function extractAmendmentReferences(content) {
    const amendments = [];
    const seenLovIds = new Set();
 ...
 */
export function parseStatuteAmendments(provisions) {
    const results = [];
 ...
 */
export function extractMetadataAmendments(metadata) {
    const result = {
        referenced_lovs: [],
    };
    // Repeal date
 ...
 *   [new text]
 */
export function parseAmendingStatute(text) {
    const sections = [];
...
//# sourceMappingURL=amendment-parser.js.map