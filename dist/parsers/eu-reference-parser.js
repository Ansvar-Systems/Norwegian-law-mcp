 ...
 */
export function extractEUReferences(text) {
    const references = [];
    const seen = new Set();
    // Extract directives
 ...
 */
function parseDirectiveMatch(match, text) {
 ...
 */
function parseRegulationMatch(match, text) {
    const fullText = match[0];
    const index = match.index || 0;
    let year;
    let number;
 ...
 */
function parseCommunity(text) {
    const normalized = text.toUpperCase().trim();
    if (normalized.includes('EURATOM'))
        return 'Euratom';
    if (normalized.includes('EØF'))
        return 'EØF';
    if (normalized.includes('EF') && !normalized.includes('EU'))
        return 'EF';
 ...
 */
function enhanceReference(ref, _text) {
 ...
 */
export function generateEUDocumentId(ref) {
    return `${ref.type}:${ref.id}`;
}
/**
 * Parse EU document ID back to components
 */
export function parseEUDocumentId(id) {
 ...
 */
export function formatEUReference(ref, format = 'short') {
    if (format === 'short') {
