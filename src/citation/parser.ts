...
/** Norwegian LOV pattern: LOV-2018-06-15-38 [§ 5], LOV-1967-02-10 [§ 1], LOV-1814-05-17-NN */
const LOV_PATTERN = /^(?:LOV\s+)?(LOV-\d{4}-\d{2}-\d{2}(?:-[a-z0-9]+)?)\s*(?:§\s*)?(\d+\s*[a-z]?)?\s*$/i;
...
export function parseCitation(citation: string): ParsedCitation {
  ...
  if (lovMatch && lovMatch[1]) {
    ...
    if (lovMatch[2]) {
      result.section = lovMatch[2].replace(/\s+/g, ' ').trim();
    }
